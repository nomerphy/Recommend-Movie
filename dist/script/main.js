...
recommendBtn.addEventListener("click", () => (progressTime = 20000));
title.addEventListener("mouseenter", moveTooltip);
...
async function recommendMovie() {
  ...
  movieAbout.textContent = `Про что «${movie.title || movie.original_title}»:`;
...
function findMovie() {
  ...
  openURL(
    `https://rezka.ag/index.php?do=search&subaction=search&q=${movieToFind} ${movieYear}`
  );
}
...
function searchInGoogle() {
  ...
  const movieYear = movieRealeseDate.textContent.slice(14, 18);
  openURL(
    `https://www.google.com/search?q=Фильм ${movieToFind} ${movieYear} смотреть`
  );
}
...
function changeProgressBar() {
  ...
  const currentProgressBarWidth = progressTime / currentTime;
  progressBar.setAttribute("style", `width: ${108 * currentProgressBarWidth}%`);
...
changeProgressBar();
let progressBarInterval = setInterval(changeProgressBar, 100);
